<template>
  ...
  >
    ...
    <div
      class="pointer-events-none z-10 flex transform-gpu flex-col gap-1 p-6 transition-all duration-300 md:group-hover:-translate-y-10"
    >
      <component
        :is="icon"
        v-if="icon"
        class="size-12 origin-left transform-gpu text-neutral-700 transition-all duration-300 ease-in-out md:group-hover:scale-75"
      />
      <div
        v-else
        class="size-12 origin-left transform-gpu text-neutral-700 transition-all duration-300 ease-in-out md:group-hover:scale-75"
    ...
    <!-- CTA: Solo visible en hover en desktop -->
    <div
      class="pointer-events-none absolute bottom-0 hidden w-full transform-gpu flex-row items-center p-4 opacity-0 transition-all duration-300 md:flex md:translate-y-10 md:opacity-0 md:group-hover:translate-y-0 md:group-hover:opacity-100"
    >
      <span
        v-if="cta"
